...
- **2026-09-23, CC:** the extraction itself has since gone further than this entry's own first
  step names. `results/genome/bank/extract_bank.py` now derives every birth id from the entity
  it names (`type_birth_id`, `pair_birth_id`, sha256-keyed, no collisions over 4,290 possible
  ids), closing [[BIRTH-IDS-THAT-ARE-NEVER-REUSED-MUST-BE-DECIDED-BEFORE-THE-FIRST-GENOME-EXISTS]]
  in `backlog_closed.md`, and the type-pair table's own regularity is now measured and read
  (`results/genome/bank/REGULARITY-READING.md`, commit `9116ff9` and `9ed0dae`), closing
  [[THE-REGULARITY-WE-MEASURE-IS-AT-THE-RULE-LEVEL-WHERE-IT-IS-MAXIMAL-AND-NOT-AT-THE-TABLE-THAT-CARRIES-THE-ENTROPY]]
  in `backlog_closed.md`. Neither closure reaches this entry's own claim: whether
  `docs/plans/2026-09-20-genome-design-around-s2.md` itself carries settled label provenance is
  untouched by either, and a fresh defect in that same design document was found today — see
  [[THE-S2-DESIGN-NAMES-THE-SHUVAEV-DECODER-AS-A-FIXED-PROJECTION-WHEN-THE-SOURCE-PAPER-TRAINS-IT]].
  This entry stays open on its own terms.
- **axis:** knowledge
...
### THE-S2-DESIGN-NAMES-THE-SHUVAEV-DECODER-AS-A-FIXED-PROJECTION-WHEN-THE-SOURCE-PAPER-TRAINS-IT: `docs/plans/2026-09-20-genome-design-around-s2.md` describes the genomic-bottleneck decoder as a fixed random projection, but the paper it cites trains it (MEDIUM, open, 2026-09-23 — recorded while reading §I.1 sources at source for the first rule proposal; filed by CC)

- **Observed.** `docs/plans/2026-09-23-first-rule-proposal.md` (commit `5a46886`) states, after reading the §I.1 sources at source (Sims, Hornby & Pollack, CPPN, HyperNEAT, and the genomic-bottleneck paper): "the S2 design describes the genomic-bottleneck decoder as a fixed random projection, while that paper's Methods describe a trained network over fixed labels."
- **Observed.** This is the same class of defect as [[THE-S2-DESIGN-CARRIES-A-CHECK-ITS-OWN-AUTHOR-HAS-WITHDRAWN]] — a design document making a claim about its own cited source that does not survive opening the source — but is a different section and a different mistake: not an internal contradiction, a mismatch with the paper it cites.
- **Inferred.** If the S2 design's decoder framing is carried into the extraction or into a rule's decode step uncorrected, a rule that should be compared against a *trained* g-network gets compared against a fixed one instead, which is an easier bar. `results/genome/c6/decoders/bf_decode.py` and the C6 harness's BF_r opponent are a trained bilinear factorisation, not a fixed projection, so the C6 exam itself does not inherit this error — but the design document that named the genome track's decoder framing still does.
- **First step.** **Ark**, who owns `docs/plans/2026-09-20-genome-design-around-s2.md`, corrects the decoder description against the genomic-bottleneck paper's own Methods section, with a dated amendment note per the checklist's rule 16 convention (repository is read-only to him; not CC's edit).
- **axis:** knowledge

### FOUR-CITATIONS-OF-ARKS-2026-09-20-21-43-RULE-WRITE-BARE-LOCAL-TIMESTAMPS-NOT-RECONCILED-AGAINST-THE-SAME-FILES-OWN-UTC-STAMPS: `results/night5/diagnostics/rowB/README.md` (lines 9, 375, 376, 417) and `results/night5/diagnostics/rowB/PENALISED-QUANTITY-READING.md:75` all cite Ark's DRIFT/SMOOTH/AMBIGUOUS rule by a bare "local" clock, while the same files also carry UTC timestamps for adjacent events with no stated offset between the two (MEDIUM, open, 2026-09-23 — found while checking the C6/genome commits for date/time handling; filed by CC)

- **Observed.** `results/night5/diagnostics/rowB/README.md:9` — "Ark, DPC Research group chat 2026-09-20 19:13 local"; `:375` — "written down by Ark on 2026-09-20 21:43 local, AFTER the reading"; `:376` — "message on the field list, 19:13 local"; `:417` — "wording Ark's, group chat 21:43 local". `PENALISED-QUANTITY-READING.md:75` — "Ark wrote it down on 2026-09-20 21:43 local — after this reading". None of the five states whose local clock it is, and the same README carries `:415` "AUTHORISED by Mike 2026-09-20 09:27 UTC" three lines below one of the "local" citations with no conversion given between the two.
- **Observed.** This is the same class of defect commit `2c095558` corrected in `results/night5/diagnostics/rowB/PROFILES-READING.md` §0 ("a quoted time that joined a local date to a UTC time is corrected, with the original wording kept in brackets") — that fix was not carried to these five citations of the same underlying 2026-09-20 21:43 event elsewhere in the repository.
- **Inferred.** Not yet shown to be a wrong reading — Bangkok local (UTC+7) is consistent with the UTC stamps at every point checked (19:13 local ≈ 12:13 UTC, after Mike's 09:27 UTC authorisation; 21:43 local ≈ 14:43 UTC, later the same day) — so this is an unreconciled-notation defect, not a demonstrated ordering error, until someone states the offset and checks all five against every UTC stamp in both files.
- **First step.** **CC** adds one line at each of the five citations naming the timezone (or the UTC equivalent in brackets, as `PROFILES-READING.md` §0 already does), and confirms no two citations of the same 21:43 event disagree once converted.
- **axis:** honesty

### PROFILES-READING-SECTION-0-HASH-DOES-NOT-REPRODUCE-FROM-ANY-COMMITTED-REVISION: the sha256 sealed in §0 of `PROFILES-READING.md` as taken before the profiles were opened cannot be recomputed from any revision of the file in this repository's history (MEDIUM, open, 2026-09-23 — recorded in the file's own dated note; filed by CC)

- **Observed.** `results/night5/diagnostics/rowB/PROFILES-READING.md` §0z (and `docs/notes/2026-09-23-translated-pinned-files.md:46,76`) records that the digest `a4d10e67f6eafa922f555625569ac5ca11db2de8bb27a01e597acbc0a945583d`, declared taken at 2026-09-20T09:31:44Z over the file as saved at 09:30:25Z, does not reproduce from any committed revision, in either line ending; the two hashes that do reproduce from commit `2488ecb` are a different pair (`d5a75ac9…` / `b2ca1f0e…`).
- **Observed.** This is the same finding already made once this session for a different file: `results/night5/diagnostics/rowB/PROFILES-READING.md`'s own pre-reading declaration (translated, corrected 2026-09-23) states plainly that "the bytes that were hashed were never committed, so the claim that the rule preceded the values cannot be checked from this repository."
...
- **First step.** No further action is owed by this entry alone — the file already states the non-reproduction and the record it would need (the actual saved bytes at 09:30:25Z, if they still exist anywhere off-repository) is Mike's or Ark's to locate, not something committing a note can manufacture.
- **axis:** honesty
...
- **Observed — the harness runs with no rule in it, and finds problems in itself first.** `results/genome/c6/harness.py` (commit `d443cf6`) implements the exam with four controls — an oracle, the null N1, a random projection, and 99 degree-preserving shuffles — and a plug-in slot for a rule that carries nothing. The controls behaved as registered. The run also found, before any rule was scored: the size-matched stored-table comparison had no teeth (N1 alone costs about twice the size limit); N1 is weaker than the simpler N0 on offset sets; the random-projection arm always scores below N1. `HARNESS-CONTROLS.md` records all three, and two harness conventions not in the original spec are named. The harness builder had not read the rule proposal.
- **Observed — the amendment was registered before it was built, on Mike's word.** `docs/plans/2026-09-23-c6-amendment-acceptance.md` (commit `4332d17`) states what the amended exam must be able to do (reject a stored table, accept a planted small non-additive rule, stay non-trivial), what it may not do (introduce monotonicity), and freezing rules for its tuned constants — written by the harness builder before the fix, under **Mike's word of 2026-09-23 21:01 UTC** to take option A (fix the exam against pre-registered acceptance criteria, rather than the alternatives). The amendment itself (commit `dd34f9d`) arms the budget arm, strengthens the offset-set target, raises P4 to a trained bilinear factorisation of the same rank, and adds a planted-rule acceptance suite; all controls re-ran with no rule.
- **Observed — 15 of 16 registered criteria pass; R1 fails and is recorded, not re-tuned.** `results/genome/c6/HARNESS-CONTROLS.md`: at N1's own size the armed table does not beat N1 — R1 fails as registered, and the entry is explicit that the failure is reported rather than fixed by moving the goalposts.
- **Observed — the narrow-band finding.** `results/genome/c6/README.md`, "What the budget arm is for" (Ark, 2026-09-23 21:35 and 21:44 UTC, translated): in the 7.7–9.5 kbit band the verdict rests on N1 and the shuffle, not on storage — R1's failure and this narrow band are recorded as one fact, home `docs/plans/2026-09-23-c6-amendment-acceptance-2.md` §2.
- **Observed — hubs, 14 of 18.** `HARNESS-CONTROLS.md` §4: H18 (top 18 types by leverage at rank k90) and U18 (top 18 types by fraction of cells unmoved by the 99 shuffles) overlap on 14 of 18 types (5.0 expected by chance, hypergeometric p = 1.1×10⁻⁷); H18 is not the same object as k90 itself (a component count, not a set of types) — P3 is reported unchanged by this finding, as registered.
...
- **Reported, not independently verified from this repository (Mike, 2026-09-23).** Mike's separate word at 21:59 UTC gave "yes to preparation" — i.e. yes to finishing A6 and BF restart parity and to measuring fitting-code timing, not yet yes to running the rule.
- **Reported, not independently verified from this repository (Mike, 2026-09-23).** Zcode, the mechanical reviewer, disclosed at 21:43 UTC that he had opened the rule proposal — a fact about a reviewer's own blindness, not about the harness, recorded here because the reviewers below must weigh it.
...
- **First step.** Finish A6 and the BF restart-parity code; implement the rule's fitting code; measure timing on shuffled bank 0; only then does Mike give his separate, explicit yes on running the rule against the exam. **The first rule run is NOT yet approved.**
- **Reviewers.** Ark (content review; has not opened the rule). Zcode (mechanical review only; disclosed 2026-09-23 21:43 UTC that he has opened the rule, so his review is mechanical-only from here). Johnny (has not opened the rule).
