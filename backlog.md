...
### A-BATCHED-GPU-INSTRUMENT-CAN-RUN-THE-C6-FITS-IN-MINUTES-BUT-IS-A-SEPARATE-INSTRUMENT: Thousands of same-shaped C6 fits (shuffles x predictors x starts) batched into one GPU tensor could cut a two-hour CPU run to minutes, but only as a separate instrument validated fit by fit against the registered numpy harness (HIGH, open, 2026-09-25 — Mike, DPC Research group, 2026-09-25 09:29-09:40 UTC: 'wouldn't it be faster on GPU?'; 'GPU is a separate instrument, not a rewrite of the CPU one'; 'in the new session we do option A')

- **Observed.** Ark measured bf_als r=4 at 23.8 ms per call with starts=1 (183.8 ms with starts=10), mostly numpy dispatch, so a straight port would be slower; llama-server holds most of the VRAM (28.6 of 32.6 GB, Ark); the knockout synthetic run took 85 min for 45 worlds on 30 CPU workers.
- **First step:** Write the batched GPU fitter for bf_als and the rule's fit; validate it against the CPU fits saved in connectome-seed-data/knockout_regrow/synthetic_rev3_prerun/raw_fits.json.gz (lambda chosen, AUC, p_P per world); VRAM freed on Mike's word.
- **axis:** reach, honesty
- **filed:** CC · 2026-09-25
...
- **2026-09-25, CC:** Synthetic-only run of revision 2 (85 min, D11 by Mike 2026-09-24 21:04 UTC) stopped on the M worlds: M1.0 read R 5/5 (a correct reading), M0.5 read G 4/5 (false absence); lambda selection is a switch (ties to lambda = 100 = N1, harness.py:727-728); U never visited. Consensus 2026-09-25 08:22-08:30 UTC (Zcode, Ark, Johnny; Mike backed CC's resolution of the two splits): revision 3 redefines G as 'not detected above gamma*' printed with its instrument; M worlds become the power curve; mechanism labels are description; degenerate shuffles are excluded from leg S; U stays only if a denser gamma grid visits it, otherwise it is renamed 'insufficient evidence (uncalibrated)'; a fixed-lambda = 1 diagnostic is printed and decides nothing.
- **2026-09-25, CC:** Session close 2026-09-25: revision 3 dense-grid synthetic run passed the two-world check (outputs in connectome-seed-data/knockout_regrow/synthetic_rev3_prerun/ with SHA256SUMS); revision 3.1 (three limits, U as threshold signature, lambda on the verdict line, reproduction requirement) committed as an unreviewed draft with the script. Next session (Mike 09:40 UTC, option A): review 3.1, then the real run on the CPU on Mike's word; the GPU instrument is built in parallel.
