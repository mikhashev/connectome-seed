...
### THE-SIMULATION-AXIS-AT-EVALUATION-IS-FORTY-STEPS-PER-ITEM-NOT-NINETEEN: `rowB_records.json` records 40 steps per item at evaluation while Ark's field list and the design text say 19 (MEDIUM, open, 2026-09-20 — CC, from `results/night5/diagnostics/rowB/rowB_records.json`)
...
- **First step.** On Mike's word: two short runs of one seed, determinism on vs off. Prediction: on → trajectories bit-identical; off → they diverge. Minutes of GPU, not a full night.
- **axis:** honesty

### ROW-B-FLOOR-SUMMARY-REPORTS-A-NONZERO-SIGMA-WHERE-THREE-PROCESSES-ARE-BITWISE-IDENTICAL: three defects in the row-B floor summary script, found by Ark reading `rowB_floor_det.json` (LOW, open, 2026-09-20 — Ark)
