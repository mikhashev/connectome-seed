...
- **Observed.** The idea's main artefact is an evolutionary tree — *"which mutation appeared, which module was inherited from the fly, what grew, where a line broke and which skills survived the change of body"* (`idea_en.md`). A tree is a relation between named things, and the names have to exist before the first birth.
...
### THE-OPENWORM-DATE-IS-CORRECTED-IN-ONE-SECTION-AND-STILL-WRONG-IN-FOUR-PLACES: `literature.md` §I.9 records that OpenWorm dates from 2011 and says so explicitly of §E and `idea_en.md`, which both still print 2014, as do README and VISION (LOW, open, 2026-09-20 — Zcode found it, §I.9 records it; filed by CC)
