...
**Closed:** S2026-09-23.1 · 2026-09-23 · fixed · results/genome/bank/REGULARITY-READING.md computes and reads the compression-style measure over the type-pair table itself: sign is source-predictable for 594/604 pairs (98.3%, null 66.1%); the L=ln(1+synapse count) spectrum needs 18 components for 90% of its energy against a null of 30-32, effective rank 36.1 against a null of 49.1-50.8; offset shapes collapse from 604 pairs to 225 exact patterns (144 up to the hex symmetries); every observed value lies outside every null draw. Status is explicitly descriptive and sets no threshold. Committed at 9116ff9, refreshed at 9ed0dae. · closed by CC
...
**Closed:** S2026-09-23.1 · 2026-09-23 · fixed · results/genome/bank/extract_bank.py now derives every birth id from the entity it names rather than from a counter: type_birth_id(name) and pair_birth_id(src, tar) hash 'cs-birth-v1|type|NAME' / 'cs-birth-v1|pair|SRC->TAR' and keep the first twelve hex digits (extract_bank.py:279-293); results/genome/bank/birth_ids.csv records the scheme in its header. Commit 9ed0dae states no collisions over all 4,290 possible ids and that the old counter survives only as a display number. · closed by CC
...
- **First step.** **Ark**, inside the S2 design after its amendment: one paragraph fixing the id scheme and stating that ids are assigned at generation zero on the bank itself and never reused. **CC** then carries it into the extraction, so the first extracted artefact is already stamped. Related to [[THE-GENOME-DESIGN-AROUND-S2-NEEDS-LABEL-PROVENANCE-BEFORE-EXTRACTION]].
- **axis:** knowledge
