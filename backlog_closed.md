...
### BIRTH-IDS-THAT-ARE-NEVER-REUSED-MUST-BE-DECIDED-BEFORE-THE-FIRST-GENOME-EXISTS: a lineage cannot be reconstructed after the fact from labels that were reassigned, so the id scheme has to be fixed and stamped on the bank at generation zero (HIGH, closed, 2026-09-20 — Ark, from `literature.md` §I.1 and §I.7; filed by CC)

**Closed:** S2026-09-23.1 · 2026-09-23 · fixed · `results/genome/bank/extract_bank.py` now derives every birth id from the entity it names rather than from a counter: `type_birth_id(name)` and `pair_birth_id(src, tar)` hash `"cs-birth-v1|type|NAME"` / `"cs-birth-v1|pair|SRC->TAR"` and keep the first twelve hex digits (`extract_bank.py:279-293`); `results/genome/bank/birth_ids.csv` records the scheme in its header (`birth_id=sha256('cs-birth-v1|...')[:12]`) and one row per type/pair. Commit `9ed0dae` ("birth ids that come from the thing they name") states no collisions over all 4,290 possible ids and that the old counter survives only as a display number. A re-extraction, another flyvis version, or a second connectome now cannot silently renumber a homologue, which was the entry's own ask. · closed by CC

- **Observed.** The idea's main artefact is an evolutionary tree — *"which mutation appeared, which module was inherited from the fly, what grew, where a line broke and which skills survived the change of body"* (`idea.md`). A tree is a relation between named things, and the names have to exist before the first birth.
- **Reported (Ark, from `literature.md` §I.7).** The systems that actually produced a readable phylogeny gave every individual an id that is never reused, assigned at birth; the ones that recycled labels can report populations but cannot report lineages.
- **Observed, and this is why it is cheap today.** Generation zero already exists and is small: the substrate's rule bank is **65 type names and 605 type-pair entries** (`ROADMAP.md` § "Not on this roadmap, deliberately", with its source). Stamping ids onto that is a one-off edit now and a migration of every recorded lineage later.
- **Inferred.** This is a design decision, not code: what an id names (a type, a rule row, a module, an individual), whether ids are dense or opaque, and what happens to an id when its referent is edited rather than replaced.
- **First step.** **Ark**, inside the S2 design after its amendment: one paragraph fixing the id scheme and stating that ids are assigned at generation zero on the bank itself and never reused. **CC** then carries it into the extraction, so the first extracted artefact is already stamped.
- **axis:** knowledge
...
**Closed:** S2026-09-23.1 · 2026-09-23 · fixed · `results/genome/bank/REGULARITY-READING.md` computes and reads the compression-style measure this entry's First step asked for, over the type-pair table itself rather than the rule layer: sign is source-predictable for 594/604 pairs (98.3%, null 66.1%); the `L = ln(1+synapse count)` spectrum needs 18 components for 90% of its energy against a null of 30–32, effective rank 36.1 against a null of 49.1–50.8; offset shapes collapse from 604 pairs to 225 exact patterns (144 up to the hex symmetries); every observed value lies outside every null draw (`REGULARITY-DECLARATION.md`, registered before the script ran, sha256 `80e1b99c…`). The reading is explicitly descriptive and sets no threshold ("Status: descriptive... gives no verdict on any encoding"), so it answers this entry's specific ask — the table-level regularity is no longer unmeasured — without itself deciding whether a rule is smaller than the weights it generates; that larger question is carried forward by the C6 exam, see [[C6-EXAM-BUILT-BLIND-AMENDED-ON-REGISTERED-CRITERIA-FIFTEEN-OF-SIXTEEN-PASS-R1-FAILS]]. Committed at `9116ff9` (first reading) and refreshed at `9ed0dae` (regenerated bank). · closed by CC

- **Observed.** At the rule level the substrate's regularity is maximal and already recorded: the bank is **identical at extent 5 and at extent 15**, so raising the lattice adds columns and not structure (`ROADMAP.md` § "Not on this roadmap, deliberately", with its source). Raising the radius is a lattice parameter, which is exactly the "not an elephant" test of `docs/notes/2026-09-20-what-is-the-genome-here.md`.
- **Reported (Ark).** The `pattern` field of the bank takes two values, which is the same fact one level down: the rule layer is small and highly regular.
- **Inferred, and this is the entry's point.** The entropy of the substrate sits in the **type-pair table** — which types connect to which, at which offsets, with which signs — and that is precisely what a grammar would have to *produce*. Its regularity has **never been measured**. So the claim "a rule is smaller than the weights it generates" is currently being supported by a measurement taken where it cannot fail, and the measurement that would make it a bet with a threshold has not been made.
- **First step.** Define and compute a compression-style regularity measure over the type-pair table (the Clune-style threshold form: the size of the smallest description that regenerates the table, against the table itself), on the bank already in the repository. No GPU, no run, no new data. Owner: **CC**, after Ark's amendment names the fields; the measure's definition is registered before it is computed, because a regularity number chosen after seeing the table is fitted to it.
- **axis:** knowledge

## 2026-09-20 — closed by CC

### THE-BOARD-CHECKER-REFUSES-ON-ROADMAP-AND-NOBODY-READS-THE-REFUSAL: `build.py --check` reports REFUSE on `ROADMAP.md` because its generated status block no longer matches the board, and the cure is regeneration, not a hand edit (MEDIUM, closed, 2026-09-20 — CC)

**Closed:** S2026-09-20.2 · 2026-09-20 · fixed · the generated status block in ROADMAP.md was regenerated from backlog.md and docs/decisions/ by the board tool itself during this session edits — the cure the refusal named, not a hand edit — and the block stays fenced by its own markers. Verified by running the check afterwards from C:\Users\mikha\Documents\dpc-messenger against this board: 0 refusals, ROADMAP.md no longer listed. · closed by CC
...
### ROW-B-EXTRACTION-ON-NIGHTS-4-AND-5-NEEDS-THREE-SETTLED-POINTS-BEFORE-THE-FIRST-VALUE: Ark's field list for row B extraction on the checkpoints of nights 4 and 5 came with three points that are cheap to fix now and unfixable after the first value is read (HIGH, closed, 2026-09-20 — Ark, group chat 19:13 local; script and first run CC's; launch an unregistered diagnostic on Mike's word)

**Closed:** S2026-09-20.2 · 2026-09-20 · fixed · commit afdadd0. All three of Ark points were settled in the instrument before the first value was read: (a) the two time axes are named separately in every record (training checkpoints against recorded steps per item); (b) both reductions are recorded side by side under their own names, the penalty central-cell read and the ablation-style full-type read, with the corrected citation of the project ablation script rather than a flyvis file that does not exist; (c) the type axis is taken from net.connectome.nodes.type and fingerprinted against the same ablation_controls.json as row A, with a non-zero exit if it ever differs, and P1prime/P2prime are reformulated for a passive recording with P2prime multiplier declared before any floor existed. Ark free falsifier ran before any profile was opened (results/night5/diagnostics/rowB/PENALISED-QUANTITY-READING.md). Row B then ran on all ten runs in both modes on Mike word; instrument, protocol, controls, records and summaries are committed under results/night5/diagnostics/rowB/ and the raw streams stay out of git by rule. What is left over is not this entry: the additive --run-columns option is opened as its own entry. · closed by CC
...
- **2026-09-20, CC:** first launch done, on the three checkpoints Ark named (`--night all --chkpts 0,8,71`, 4 runs × 3 checkpoints = 12 pairs, 44 s, commit effaff2 holds the script; outputs untracked in `results/night5/diagnostics/rowB/`). First attempt died on a CC script bug: flyvis installs CUDA as the default device, and the reduction tensors were created on CUDA while the activity had already been moved to CPU; fixed in the working tree, uncommitted. P0 (evaluation without recording reproduces the stored checkpoint value) is 12/12 pass, with zero difference at every checkpoint 0 and small nonzero float32 differences at checkpoints 8/71, all inside its 0.001 tolerance. P1' (the recording hook does not change the loss) as registered — tolerance 1e-6 — FAILS on 7 of 12 pairs; `activity_mutated` is false (asserted by the hook itself, not assumed). The type axis is identical to row A's. Neither the profiles nor the penalised quantity has been read by anyone yet. The floor has not been taken, so P2' still has no threshold. Scope fact, restated: nights 4 and 5 are 4 runs and 3 individuals (seeds 0, 3, 5) across netdirs `9991/005`, `9991/903`, `9992/000`, `9992/003` — not 6 individuals; extending to all 10 runs is Mike's open decision.
- **2026-09-20, CC:** scope extended to all ten runs on Mike's word ("10"). `rowB.py` gained a loss-level floor, a floor taken K times (`--floor-k`), a deterministic mode writing parallel `_det` outputs, instrument labels in every record (`reduction_path`, `deterministic`, `determinism_mode`), and the P2' multiplier — 10 × the measured floor — declared before any floor existed. The registered P1' tolerance (1e-6) was not amended. **Three-checkpoint run** (`--chkpts 0,8,71`, ten runs, GPU reduction path, K = 5; copy kept in `results/night5/diagnostics/rowB/three_checkpoint_gpu_path/`): normal mode P0 30/30, P1' 13 pass / 17 fail; deterministic mode P0 30/30, P1' **30/30**, with hook-vs-no-hook difference and no-hook floor both exactly `0.0` at the per-item and at the loss level in all 30 records; no operator refused determinism. **Floor across three fresh processes** (`rowB_floor.json`, `rowB_floor_det.json`, pair 9992/003 @ 71): normal mode — loss-level no-hook floor 3.4e-5 to 3.9e-5 inside each process, and activity means (`mean_by_type`) differing between processes by at most 2.8e-7; deterministic mode — bitwise zero on both. **Full run** 72 × 10, normal mode: 720 pairs, script wall 747 s, P0 720/720, P1' 92 pass / 628 fail, `activity_mutated: false`. The same three checkpoints inside the full run give P1' 14 pass / 16 fail against 13 / 17 in the separate three-checkpoint run — two evaluations of the same states in the mode where evaluation is not repeatable, so the count itself is not a stable number; recount by a second agent found this. Deterministic full run started the same night. The penalised quantity was read once (`PENALISED-QUANTITY-READING.md`); **activity profiles have not been read by anyone.** First-run CPU-path outputs are kept in `superseded_cpu_path_first_run/` and are not pooled with anything. <!-- no-refs -->
- **2026-09-20, CC:** Ark's DRIFT/SMOOTH/AMBIGUOUS branch rule (group chat, 21:43 local, written
  after the reading, to record the rule as applied) is now in the repository:
  `results/night5/diagnostics/rowB/README.md`, "How the quantity is read across the 150,000
  boundary (rule)" (§8).
- **axis:** knowledge

### THE-SIMULATION-AXIS-AT-EVALUATION-IS-FORTY-STEPS-PER-ITEM-NOT-NINETEEN: `rowB_records.json` records 40 steps per item at evaluation while Ark's field list and the design text say 19 (MEDIUM, closed, 2026-09-20 — CC, from `results/night5/diagnostics/rowB/rowB_records.json`)

**Closed:** S2026-09-20.2 · 2026-09-20 · fixed · resolved by two independent readings of the source (CC and Ark). tools/.venv/Lib/site-packages/flyvis/datasets/sintel.py:400-402 constructs Interpolate to resample the original framerate to 1/dt, and :537-539 place that resampler outside the augmentation switch; the class is MultiTaskSintel, which training and evaluation share — so 19 config frames become 40 recorded steps per item, ceil(50/24 x 19) = 40, and the penalty window row B reproduces is the last three quarters of them. The 19-frame bin-slicing at sintel.py:847-866 belongs to AugmentedSintel, which the runs did not use, which was Ark correction. Caveat carried forward in results/night5/diagnostics/rowB/PENALISED-QUANTITY-READING.md section 7 caveat 1, not in this entry: training computed the quantity on augmented training batches, row B on held-out items with augmentation off. · closed by CC

- **Observed.** `rowB_records.json` carries `n_frames: 40` (recorded steps per item) and `n_frames_config: 19` (the config's pre-resampling count) for all 12 records. The activity penalty discards the first `n_frames // 4` frames: `activity_mean = activity[:, n_frames // 4 :, self.central_cells_index].mean(dim=1)` (`tools/.venv/Lib/site-packages/flyvis/solver.py:870-872`).
- **Inferred.** The "penalty window" row B reproduces is steps 10..39 of 40. Whether training computed its window on the same step count is NOT verified, so the recorded `central_cell_mean_skip_first_quarter` may not yet be called "exactly the penalised quantity".
- **First step.** Read how the training loop builds the activity tensor the penalty sees (its frame count and dt) and state it in `results/night5/diagnostics/rowB/README.md` — by a later edit, not CC's.
- **2026-09-20, CC:** resolved by reading, independently by CC and Ark. 19 is film frames at 24 Hz; the dataset resamples to `1/dt = 50 Hz`; `ceil(50/24 × 19) = 40`. Verified at source, `tools/.venv/Lib/site-packages/flyvis/datasets/sintel.py`: `Interpolate` is constructed at **lines 400-402** (`self.piecewise_resample = Interpolate(self.original_framerate, 1 / self.dt, mode="nearest-exact")`), inside `class MultiTaskSintel` (starts line 182) — the class training and evaluation share. The resampler sits **outside the augmentation switch**: **lines 537-539** read `# note: these two are not affected by augment` immediately above `self.piecewise_resample.augment = self.resampling`. Scheduler dt is constant in the run's `_meta.yaml` per the entry as filed (not independently re-checked here). **Ark's correction confirmed:** the bin-slicing code — temporal splitting into `ceil(sequence_length / n_frames)` bins — sits at **lines 847-866**, inside `_build()`, which belongs to `class AugmentedSintel` (starts line 733, so 847-866 falls inside it, not inside `MultiTaskSintel`). The runs used `MultiTaskSintel`, so that bin-slicing code does not apply to them; the resampler that does apply (400-402, 537-539) is the one shared and unconditional. Caveat carried forward unresolved: training computed the quantity on augmented training batches of 4; row B on 16 held-out items, batch 1, augmentation off (`PENALISED-QUANTITY-READING.md` §7 caveat 1). **Proposed closure text (not applied — leaving open):** `**Closed:** S<session> · 2026-09-20 · fixed · sintel.py:400-402/537-539 (Interpolate, resampled 24→50 Hz, outside the augment switch, shared by MultiTaskSintel training and evaluation) confirms 40 steps/item, ceil(50/24×19)=40; the 19-frame bin-slicing at sintel.py:847-866 belongs to AugmentedSintel, which the runs did not use. Open caveat carried to a new entry if needed: training's window was computed on augmented batches, row B's on held-out items.` <!-- no-refs -->
- **axis:** knowledge

### A-TRAINED-CHECKPOINTS-EVALUATION-IS-NOT-BITWISE-REPEATABLE-BETWEEN-TWO-CALLS-IN-ONE-PROCESS: evaluating the same checkpoint twice in one process does not return the same per-item loss — expected, since evaluation mirrors the nights' --no-determinism, but never measured and not allowed for in P1', and the recording hook is not the cause (MEDIUM, closed, 2026-09-20 — CC, from `rowB_controls.json`, the 2026-09-20 launch of `results/night5/diagnostics/rowB/rowB.py`)

**Closed:** S2026-09-20.2 · 2026-09-20 · disproved · the non-repeatability is bitwise zero under torch.use_deterministic_algorithms(True) + cudnn.deterministic — hook-vs-no-hook difference and no-hook floor both exactly 0.0 in every pair of the deterministic row B run, and 0.0 across all three fresh processes (results/night5/diagnostics/rowB/rowB_controls_det.json, rowB_controls_floor_proc1_det.json, rowB_controls_floor_proc2_det.json, rowB_controls_floor_proc3_det.json); no operator refused determinism. The mechanism was GPU operation order under --no-determinism, which is the nights own setting, and P1prime passes as registered once determinism is on — its tolerance was never amended. Ark, whose control P1prime is, accepted the reading in the group thread on 2026-09-20. · closed by CC

- **Observed.** Beside each hook-vs-no-hook difference, the script also records a no-hook-vs-no-hook floor — the same evaluation run twice with no hook at all. At checkpoint 0 both the floor and the hook-vs-no-hook difference are zero for all four runs (`rowB_controls.json`, the `('…', 0)` entries). At checkpoints 8 and 71 the no-hook floor is non-zero in all 8 of those pairs, on the order of one float32 ULP of the per-item loss, and the with-hook difference equals that floor in 6 of those 8 pairs. Of the remaining two: at `9991/005` checkpoint 71 the with-hook difference is 2 ULP against a 1-ULP floor (`per_item_max_abs_diff` 0.0009765625 vs floor 0.00048828125); at `9991/903` checkpoint 8 the with-hook difference is smaller than the floor (`per_item_max_abs_diff` 6.103515625e-05 vs floor 0.00048828125, 0.125 ULP). Counting checkpoint 0's trivial 0=0 pairs together with checkpoints 8/71, the with-hook difference equals the no-hook floor in 10 of 12 pairs, not 11 — the handover's "11 of 12, one pair 2 ULP against 1" undercounted the exceptions by one; the second exception is not an over-floor pair but an under-floor one.
- **Inferred.** The recording hook is not what moves the number; the evaluation itself is not bitwise repeatable across two calls in one process. This has never been measured before today. The P1' tolerance of 1e-6 is below one float32 step for a loss of this magnitude, so it was unattainable by construction — but it was set before the values existed, and CC has NOT rewritten it. Whether P1' becomes "the with-hook difference does not exceed the no-hook floor" is Ark's call, since P1' is his control.
- **2026-09-20, CC — correction to this entry's first wording.** It said "with determinism on". It is off, by design: `results/night2/diagnostics/diag1_eval_paths.py:90-93` sets `cudnn.deterministic = False` to mirror the nights, which ran with `--no-determinism` (`tools/night/start_night.ps1:68`, `tools/night/run_individual.py:285`). So the non-repeatability has a known source and is not a defect of the evaluation; what remains open is only that P1' was given a tolerance this setting cannot meet. Confirmed independently the same day: two GPU runs of identical row B code disagree in `central_cell_mean`, a pure index with no summation. CC also reported "determinism on" in the group chat and corrected it there.
- **First step.** Find the source of the non-repeatability — which op, whether `torch.use_deterministic_algorithms` covers it, and whether it is the steady-state call or the item loop — with a two-call test on one checkpoint; then Ark restates P1'.
- **2026-09-20, CC:** source confirmed by experiment, not just inferred. `rowB_controls_det.json` (30 entries, `deterministic: true`, `determinism_mode` recording `torch.use_deterministic_algorithms(True)` + `cudnn.deterministic`) shows the with-hook difference and the no-hook floor both exactly `"0.0"` in 30 of 30 pairs, and the three fresh-process floor files under determinism (`rowB_controls_floor_proc{1,2,3}_det.json`) each read `max_difference_loss: 0.0` — bitwise zero across three processes, not just within one. P1' as registered (tolerance 1e-6) passes 30/30 in this mode; its tolerance has not been amended (`tolerance_status` in the file still reads "REGISTERED and NOT amended on 2026-09-20"). No operator refused determinism: the run exited 0, not 4, and its log carries no traceback (checked by CC at launch, 2026-09-20). **Proposed closure text (not applied — Ark's control):** `**Closed:** S<session> · 2026-09-20 · disproved · the non-repeatability is bitwise-zero under torch.use_deterministic_algorithms(True) + cudnn.deterministic, 30/30 pairs and 3/3 fresh processes (rowB_controls_det.json, rowB_controls_floor_proc{1,2,3}_det.json); the mechanism was GPU operation order under --no-determinism, matching the nights' own setting, and P1' passes as registered once that setting is on.` Left open pending Ark's word.
- **axis:** honesty

### ROW-B-FLOOR-SUMMARY-REPORTS-A-NONZERO-SIGMA-WHERE-THREE-PROCESSES-ARE-BITWISE-IDENTICAL: three defects in the row-B floor summary script, found by Ark reading `rowB_floor_det.json` (LOW, closed, 2026-09-20 — Ark)

**Closed:** S2026-09-20.2 · 2026-09-20 · fixed · commit afdadd0 — the sigma guard (zero range -> sigma exactly zero), the `floor_k_inside_each_process` rename, the `_det` glob fix so the normal-mode summary no longer picks up the other mode, and the driver label in `gpu_instrument_labels` (rowB.py), with the witness file results/night5/diagnostics/rowB/instrument_stack.json; the pre-guard summaries are kept beside the current ones in floor_summaries_before_sigma_guard/. All four were made after the deterministic full run finished, so no script hash changed mid-run (Ark's condition). · closed by CC

- **Observed (Ark).** `max_sigma_by_type` reads ~2e-15 in the deterministic floor summary where the three source processes are bitwise identical (the `_det` floor-process files each report `max_difference_loss: 0.0`, confirmed independently this session from `rowB_controls_floor_proc{1,2,3}_det.json`) — a numpy `std(axis=0, ddof=1)` computed over identical float64 values via a rounded mean, in `rowB.py` (Ark's citation; I did not independently grep the exact line numbers in `rowB.py`, which is outside the files I was told I may read this session).
- **Observed (Ark).** `floor_k` in the floor summary means "floor repeats inside a process" in the controls file's own usage (`rowB_controls*.json`, `floor_k` in `env`/top level) but carries a meaningless default value in the floor summary context, where it does not track anything real.
- **Inferred.** Both are instrument-labelling defects rather than data defects: the underlying three-process agreement is genuinely bitwise (confirmed above), the summary script just fails to say so cleanly.
- **First step.** Guard the sigma computation (zero range → sigma exactly zero, not a rounding artefact of an identical-value std); rename the meaningless `floor_k` default in the floor-summary context; add the CUDA driver version to the instrument labels; add the README line that in deterministic mode P2' degenerates to exact equality, stated on the same line as the threshold (Ark). To be done after the deterministic full run finishes, so the script hash does not change mid-run.
- **2026-09-20, CC:** the fixes are in the working tree, uncommitted — the sigma guard (§6, zero range → sigma exactly zero), `floor_k_inside_each_process` (§6a rename), the `_det` glob fix, and the driver-label code (`gpu_instrument_labels`, `rowB.py:254`) plus a same-day witness file `results/night5/diagnostics/rowB/instrument_stack.json`. Not closed here: closure is after commit.
- **axis:** honesty
...
- **Was still open when this bullet was written, 2026-09-18 — kept as the record of what was true then, and NOT the ground of the closure below (Ark asked that the boundary be unambiguous, 2026-09-19).** Ark's acceptance condition was unmet by construction: it requires a **full run's** `json_iter_wall_median_all_s` to land inside **[0.0598, 0.0622]**, which no dry run can supply. The entry stays open until a night has run from `tools/.venv` and its timing gates are read (night 5's §4 gate 4 is the instrument). Also unresolved: the scratchpad copy is still the only place nights 1-4's raw artefacts exist, and deleting it is a separate decision from moving the environment.
...
(`results/diagnostics/gray/README.md:229-231`); summarised in `ROADMAP.md` § "Diagnostics while N accrues — the functional-readout track", step 1 · closed by CC
...
### THE-MACHINE-VERDICT-ON-THE-NIGHT-3-ABLATION-READING-WAS-OVERRIDDEN: ablation_reading.json's own POSITIVE verdict evaluated only one of the pre-registered rule's two conflicting size definitions, and R2 itself did not repeat (HIGH, closed, 2026-09-16 — Mike "let's try it", translated from Russian; reviewers Ark 05:27Z and Zcode 05:27Z concurring)
...
### NIGHT-3-RUNS-SEEDS-3-AND-4-ON-MIKES-WORD: seeds 3 and 4 trained to 250,000 in one wave with no reboot, the second same-wave pair after seeds 0/0prime, pushing the rung SD to n=5 (HIGH, closed, 2026-09-16 — Mike, DPC Research chat 2026-09-16T04:46Z: "The night run has finished", translated from Russian)
...
- **Observed.** Mike's word (DPC Research chat, 2026-09-15 09:56 UTC: "do row B", translated from Russian). Protocol:
...
### RESEARCH-REPO-START-CHECKLIST-WRITTEN-ON-MIKES-WORD: fourteen rules distilled from this session's incidents, so a new research repository starts by reading them rather than re-deriving the principles (LOW, closed, 2026-09-15 — Mike 09:56 "write the checklist", translated from Russian; drafted by Ark 08:50/09:39/09:51, written by CC)
...
- **Observed.** Ark proposed the checklist three times (08:50, 09:39, 09:51) as the rules
  ...
  chat, 2026-09-15 09:56 UTC): "write the checklist" (translated from Russian). The file carries the same 14 items in the same
...
**Closed:** S2026-09-13.1 · 2026-09-15 · fixed · seed 1 (9991/001) EXIT rc=0, 250,008 iterations, 14,532.9 s; seed 2 killed at iteration 12,700 by the KB5129195 restart, partial dir renamed to flow/9991/002_killed_by_reboot, excluded under §7's resume rule; re-run as wave night2b (9991/002), started 2026-09-15T01:13:46Z, EXIT rc=0, 250,008 iterations, 14,296.6 s, done 05:12:18Z; Mike, chat 05:19Z: "the run has finished", translated from Russian; record docs/experiments/002-night2-seeds-1-and-2.md and results/night2/ · commit: (this commit) · closed by CC
...
- **Observed 2026-09-15 (CC).** Mike's word given (DPC Research chat, 2026-09-15 18:32 local:
  "put them in, yes, and write me the command…", then "launched" — both translated from Russian); launched. Launcher log: `WAVE START
...
**Closed:** S2026-09-13.1 · 2026-09-15 · fixed · Mike decided yes 2026-09-15 18:05 local, DPC Research chat: "ok, let's write it in", translated from Russian; the form is written into docs/preregistration-cheap-vs-expensive.md as secondary hypothesis (b2) — §1 bullet, §4 statistic X = |Top-k(C3) ∩ Top-k(250,000)| with exact hypergeometric tails for N = 8/10/12/16/20 and k = 2/3 from docs/prereg-scripts/topk_tail.py (8!-permutation self-check passed, Python 3.12.10), §5 decision rule and timing rule, §7 measurability paragraph, §8 exclusions, §9 provenance; k = 2 fixed, marked (proposed — fix in review) until seeds 1 and 2 start · commit: (this commit) · closed by CC
...
### IDEA-MD-IS-VERBATIM-RUSSIAN-IN-A-REPOSITORY-THAT-OPENS-IN-ENGLISH: the idea is 62 % Cyrillic by character because it is the author's text as written, and the public version must be English without ceasing to be the record (LOW, closed, 2026-09-13 — Mike, 06:46 UTC: "everything there has to be in English", translated from Russian)
...
### ONE-GPU-ITERATION-BEFORE-ANY-OVERNIGHT-RUN: the price of the expensive evaluation on this card is an estimate until one training iteration has been timed here (HIGH, closed, 2026-09-13 — Mike, 07:20 UTC: "check the whole flow first, and the long runs at night", translated from Russian)
...
### PRE-REGISTER-THE-CHEAP-VERSUS-EXPENSIVE-TEST: the test has a verified flow and no written reading, so a run today would produce a number nobody could interpret (HIGH, closed, 2026-09-13 — Mike, 07:27 UTC: "you do everything, we review", translated from Russian; the demand for numbers first is Johnny's, Ark's and Warren's, independently)
...
### THE-ATTRIBUTION-STRING-FOR-CC-BY-IS-NOT-WRITTEN: the licence is chosen and the LICENSE file cannot be written without saying who the author is (MEDIUM, closed, 2026-09-13 — Mike, 06:55 UTC: "ok CC BY 4.0", translated from Russian)
...
- **Observed.** All three entries in the transcript are attributed to `Mike Shevchenko`; Mike
  stated he showed the idea to Grok. The name "Beznogim" (Russian, "Legless") does not occur anywhere in this
...
### THE-NIGHT-RUN-STARTS-ONLY-ON-MIKES-EXPLICIT-WORD: the launcher is ready and dry-printed, and nothing starts until Mike says so (HIGH, closed, 2026-09-13 — Mike, 16:22 UTC: "we do not launch the full night run until I say so explicitly", translated from Russian)
