...
the conclusion, that row A and the weights agree and row B reverses them, is unchanged.) **Narrowed (Ark 10:30, on the corrected labels):** the swap was in two columns of the chat table (row A and the weights) — values were sorted and labels attached afterwards; row B carried its labels correctly. With the correct labels the middle pair in row A is a tie: (0′,2) 0.55708 vs (1,2) 0.55979 differ by 0.0027, so the order between them is not established. What is established: (0,2) is the farthest pair in row A (0.64663) and in the weights (0.78) and the closest in row B (0.546) — the inversion is made by row B and made exactly on the (0,2) pair, the same pair that flips from farthest at iteration 0 to closest at 250,008; 0.6466 vs 0.546 is the largest disagreement between the two channels among the six pairs. "A agrees with the weights, B is the exact reversal" is therefore narrowed to that one pair; the rest is noise and middle-order. Over the five foreign pairs the Spearman correlation between row-B distance and weight
