---
**Status: DRAFT, revision 1, 2026-09-26 UTC (`date -u` read 10:26 UTC when drafting began). Not
reviewed, not voted, not committed. Nothing was fitted for it: no GPU run of the instrument, no
CPU fit. The only runs made while drafting are listed in §12 (a 20-second environment probe on
random tensors, and read-only counts and checksums over the pinned and the flyvis-65 run's
synthetic stores).** Drafted by a CC
...
| M10 | v3 resources | prepared arrays 2,209 MiB; torch peak 12,526 MiB; `nvidia-smi` peak 18,176 MiB (other processes included); GPU utilisation mean 98.4 % | `run7_pipeline_all45.log:37–57` | as recorded |
...
| M15 | λ of the shuffled banks | λ = 100 in 4,291 / 4,416 / 4,446 / 4,450 of 4,455 (BF_1–BF_4); near-ties (a λ within 1e-7 of the tie threshold) in 1,586 / 1,665 / 1,689 / 1,678 of 4,500 banks; none flipped | store (§12, P3); `run7_pipeline_all45.log:1–4` | verified (λ); as recorded (near-ties) |
...
4. **Which fits differ depends on the batch.** v2 with the 45 base views in one batch misses 3
   ...
   run saved the per-fit `p` (§11, row G-(4)).
...
- **Batch sensitivity.** `run4_all45batch.txt` holds the same three views fitted inside a 45-view
  ...
  `run1.txt:2–10`); the BF_4 hashes and every λ are equal. The script that produced `run4` is not in
  the folder (`probe_once.py` fixes the three worlds, :32), and the file has no stamp line (no
  torch version, device, chunk or `det`), so its producer cannot be checked.
- **Root cause (Zcode, chat 09:32).** Not float32; float64 reduction order (cuBLAS / cuSOLVER
  ...
  2^-24)" is not exact: `p` is float64, and 2^-24 = 5.96e-8 (§11, row G-(5)).
...
any order on any worker. Three consequences are registered here: the batch composition is part of
the instrument and is recorded and fixed (D5, §6); equivalence to the CPU is claimed on the
deciding outputs, not on the bits of every `p` (D3, §5); and the explanation of the differences is
tested, not only stated (V5, §7).
...
| **D12** | Go or wait | **(a) validate the BF-only hybrid now** (§7, about 2 h 10 min of GPU): it saves about 49 min per synthetic pass of a flyvis-65-sized arm (§9). (b) wait until rule #2.1 is faster on the GPU: the saving would grow toward about 65 %, but no port exists that is faster (M14). (c) drop it | **(a)** |
...
**E2, the `p` bound.**
- Base views: at most **5 of 180** BF fits not bit-equal, each with max |Δp| ≤ **6e-8** over its
  64 cells.
- Shuffles: **0 of 17,820** not bit-equal (the measured value, twice, under two compositions: M1,
  M2). A shuffle difference within 6e-8 is a finding that refuses the instrument until reviewed; it
  is not tolerated silently.
- The set of differing base fits is printed. Under D6 it may differ from M3's five; a different set
  within the bound passes and is reported.

**E3, the continuous columns, within a propagated bound.** A's gate compares `D`, `logloss`,
`logloss_margin_over_N1`, `regrown_share_full`, `regrown_share_block` within `MACHINE_CHECK_TOL`
= 1e-9 (`knockout_regrow.py:319`, `:2269–2270`). That tolerance was set for two CPU runs and is
too tight for this comparison: at p = 0.5, a Δp of 5.6e-8 on one cell moves `D` (the mean logit
over present cells minus the mean over absent ones, `knockout_regrow.py:478–483`) by
5.6e-8 / (0.25 × 32) ≈ 7e-9. The bound is therefore computed per row from the pinned `p` with
|Δp_i| ≤ 6e-8 on every cell:
- `D`: Σ_{present} 6e-8 / (n_present · p_i (1 − p_i)) + Σ_{absent} 6e-8 / (n_absent · p_i (1 − p_i));
- `logloss` and `logloss_margin_over_N1` (N1's part is a CPU fit, identical): the mean over the 64
  cells of 6e-8 / p_i (present) or 6e-8 / (1 − p_i) (absent), with p clipped to `CLIP` =
  (0.001, 0.999) (`harness.py:65`, `:493–494`);
- the two regrown shares are ratios of exact columns and must be exact.

On the pinned store these bounds are 6.2e-7 to 8.7e-7 for `D` and 1.1e-7 to 1.4e-7 for `logloss`
on M3's five rows, and at most 4.3e-6 (`D`) and 4.9e-7 (`logloss`) over all 180 BF base rows (§12,
P5). **Prediction, written before V3:** A's gate with its 1e-9 tolerance would report outcome 3(b)
on up to five BF_1 rows; E3 is the comparison that decides for this instrument.

**Outcomes.** (1) E1, E2 and E3 hold: the instrument is equivalent on A's worlds, and the result,
with the set of differing fits, is committed. (2) E1 holds, E2 or E3 does not: refused; the
reviewers decide whether a revision is needed. (3) E1 fails anywhere: refused; the instrument
changed a deciding output.
...
- **R1, environment stamp** (D7): the registered stamp equals the run's, field by field; the
  manifest records it (G2).
...
- **R4, batch composition** (D5): the digest of the ordered key list and of `row_chunk`, `starts`,
  the rank order and every chunk boundary equals the registered one; free VRAM is checked before
  the upload against the registered need (the measured torch peak, 12.5 GB at `row_chunk` 40,000,
...
| V1 | self-stability: three full independent runs of the registered composition | A's 45 worlds, 99 shuffles + base, BF_1–BF_4 (18,000 fits) | about 3 × 40 min = 2 h (plus 3 × 2.3 min of 24 CPU workers) | all 18,000 × 3 per-fit hashes equal | any hash differs: "NOT DETERMINISTIC" |
| V2 | equivalence, E1 and E2 | V1's first run against A's pinned pre-run store | none (CPU, minutes) | E1, E2 of §5 | any λ, label or AUC difference; E2 exceeded |
...
| V4 | batch sensitivity, a statement, not a gate | (a) the 45 base views as one batch; (b) one world per batch for M1.0:0, M0.85:2, M0.75:0; (c) optional: all 45 worlds at `row_chunk` 100,000 | (a) about 30 s; (b) about 3 min; (c) about 37 min at 26.5 GB (README:301) | the report: how many per-fit hashes change against V1, and E1–E3 against the CPU for each composition | E1 fails under a composition: not a refusal of the registered composition, but it is reported, and D5 (a) is then the only safe option |
| V5 | the explanation of the base differences (Ark's condition) | (a) CPU only: `harness.fit_bf` on the 180 base views with `OPENBLAS_NUM_THREADS=4` exported before launch (another BLAS reduction order), compared with the pinned store; (b) engine v3 on `torch` CPU device, float64, on M3's five fits and five bit-equal controls | (a) none (CPU, a few minutes); (b) none (CPU, minutes) | written before the run: if the fits that move in (a) are mainly the rank-1, small-λ base fits (M3's five or a set overlapping them) while most of the rest stay bit-equal, the differences are a property of those fits under any change of reduction order, and the tolerance of E2 is explained | (a) moves none of M3's five and (b) reproduces them: the differences come from the GPU code, not from the fits; E2 is then not registered, and the instrument waits for a fix |
| V6 | negative control: the comparator can separate | engine v3 with `BF_TOL` 1e-5 (instead of 1e-6), R:0 base + 99 shuffles | about 1 min | the comparator flags the run (E2 or E1) | the comparator passes a run made with a changed stopping rule: it cannot separate, and it is refused |
...
**Totals.** V0–V7: about **2 h 10 min of GPU time** (about 2 h 50 min with V4 (c)), mostly V1; CPU
minutes. V8: about 80 min more, after the male pre-run's store exists. The GPU runs use 24 CPU
...
revision of this file, which registers the composition digest, the stamp and the set of differing
base fits, and is reviewed before any arm names the instrument.
...
- **The male CNS arm:** stays on the CPU: its D13 keeps (i) and allows (iii), the unregistered
  ...
  validation (about 2 h 10 min of GPU plus a review) is longer than the saving on one lobe's pass
...
| the validation of this file (once) | — | ≈ 2 h 10 min of GPU, CPU nearly free | a cost |
...
**Break-even.** The validation costs about as much GPU time as 1.3 arms save; the larger cost is
...
deciding output exact); the `p` values themselves may differ within E2 and E3 on a few base fits.
...
| G2 | the environment record in the manifest: Python, numpy (`show_config`, `threadpoolctl`) in the main process and in one prep worker, torch, `torch.version.cuda`, cuDNN, the sha256 of `cublas64_12.dll`, `cublasLt64_12.dll`, `cusolver64_11.dll` in `torch/lib`, GPU name, compute capability, total VRAM, driver (`nvidia-smi`), the determinism settings, git head, `tree_dirty_paths`, the LF sha256 of every instrument file and of A's pinned files; the stamp check of D7 (i) (R1) |
| G3 | the batch manifest: the ordered key list, `row_chunk`, `starts`, rank order, every chunk boundary, and their digest; refusal on a digest other than the registered one; VRAM preflight; no chunk fallback (R4) |
...
| G9 | `gpu_equivalence.py`: E1, E2, E3 of §5, reading A's pinned pre-run store after `check_prerun_files` (imported read-only from A's script); E3's bounds computed from the pinned `p` |
...
- **T-G3:** the composition digest changes when the key order, `row_chunk` or `starts` changes, and
  not otherwise; a digest mismatch refuses.
- **T-G4 (the comparator separates both worlds):** on the pinned records, an injected Δp of 1e-7 on
  one cell fails E2; 6 of 180 differing base fits fail E2; one differing shuffle fails E2; a λ
  changed on one fit fails E1; a label flipped fails E1; a `D` moved beyond its E3 bound fails E3;
...
- **T-G7:** two subprocess runs of one world's base view and 5 shuffles give equal hashes (seconds
  of GPU).
- **T-G8:** the arm's path check (G12) passes a BF record within E2 and fails one outside it or with
...
| G-(3) | "the GPU scripts have no thread preamble — add" | the v2/v3 drivers have one (row G-(2)); what none has is a record of the environment actually in effect: they record `gpu_name`, `workers` and `row_chunk` only | chat only (Zcode, 09:32 UTC, as relayed) | `run_pipeline.py:25–26`, `:184–186`; `validate_shuffles.py:130–133` | this draft |
| G-(4) | "v2 and v3 disagree on 2 of 180 base fits" | at least 3: M1.0:0 and M0.85:2 (bit-equal in v2, not in v3) and M1.0:4 (not bit-equal in either; max \|Δp\| 5.54e-8 in v2, 3.16e-8 in v3, so the two `p` differ); M0.75:0 and M0.85:0 cannot be told apart without the per-fit `p`, which neither run saved | chat only (as relayed in the drafting brief) | `validation_shuffles_base45.json` (row `world:M1.0:4\|\|ko\|\|BF:1`, 5.540526e-08); `run7_pipeline_all45.log:95` (3.156e-08) | this draft |
| G-(5) | "Δp = one float32 ulp (5.63e-8 ≈ 2^-24)" | `p` is float64, computed from float32-cast parameters (`harness.py:273–277`, `decoders/bf_decode.py:10`); 2^-24 = 5.96e-8, not 5.63e-8; the size fits one float32 rounding step of a cast parameter carried through the sigmoid, which no file shows | chat only | `run7_pipeline_all45.log:11` (5.6323528e-08); arithmetic | this draft |
...
- How `run4_all45batch.txt` was produced (no script, no stamp line).
