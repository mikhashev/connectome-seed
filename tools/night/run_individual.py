...
def _machine_state():
    ...
    try:
        if isinstance(state.get("uptime_s"), (int, float)):
            _boot_ts = time.time() - state["uptime_s"]
            state["boot_time_utc"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(_boot_ts))
        else:
            state["boot_time_utc"] = {"error": "uptime_s unavailable"}
    except Exception as e:  # noqa: BLE001
        state["boot_time_utc"] = {"error": repr(e)}
    try:
