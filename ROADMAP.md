...
lands on (`decisions/`) and the tasks it consists of (`backlog.md`). Dates are the writer's
...
**Decisions:** [001 — publication shape](decisions/001-publication-shape.md) (CC BY 4.0;
transcripts never enter history). [002 — file under a condition](decisions/002-file-under-condition.md)
...
`decisions/`; it is the only status in this file that is not typed by hand, so it is the only
